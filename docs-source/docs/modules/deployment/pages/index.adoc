...
The Akka Cloud Platform is currently available in xref:aws-index.adoc[Amazon Web Services (AWS)]

== Getting Started with Cloud Deployment
Before you can get started with cloud deployment you should:

. Have an active account with AWS.
. Subscribe to the Akka Cloud Platform with xref:aws-index.adoc[AWS].
