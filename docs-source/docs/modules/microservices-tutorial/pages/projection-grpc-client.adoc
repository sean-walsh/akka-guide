...
// example for #2: docker exec -i shopping-cart-service-scala-postgres-db_1 psql -U shopping-cart -t < ddl-scripts/create_tables.sql
2) docker exec -i <<image id>> psql -U shopping-cart -t < ddl-scripts/create_tables.sql

3) sbt docker:publishLocal
4) kubectl apply -f kubernetes/namespace.json
5) kubectl config set-context --current --namespace=shopping-cart-namespace
6) kubectl apply -f kubernetes/akka-cluster-roles.yml
7) kubectl apply -f kubernetes/akka-cluster.yml
...
. akka-cluster-roles.yml
+
[source, shell script]
----
include::example$shopping-cart-service-scala/kubernetes/akka-cluster-roles.yml[]
----
