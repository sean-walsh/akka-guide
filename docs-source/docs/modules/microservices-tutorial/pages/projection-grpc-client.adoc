...
. Define the interface of the service in a protobuf service descriptor. It should be located in the `src/main/protobuf/ShoppingOrderService.proto` in the new `shopping-order-service` projjjt.
+
[source,protobuf]
----
...
// TODO: Add short description of kubernetes and how to run the app based on akka-cluster.yml and namespace.json links.
// below is an example

=== Kubernetes option
. As an alternative, you can run the project using https://kubernetes.io/[Kubernetes] with the following command and configuration files:
+
[source, shell scrpt]
----
kubectl -f kubernetes/akka-cluster.yaml
----
